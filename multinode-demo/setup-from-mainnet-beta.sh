...
(
  ...
  set -x
  wget https://api.mainnet.nexis.network/genesis.tar.bz2
  wget --trust-server-names https://api.mainnet.nexis.network/snapshot.tar.bz2
)
...
if [[ -r $FAUCET_KEYPAIR ]]; then
  ...
  $nexis_keygen new --no-passphrase -fso "$SOLANA_CONFIG_DIR"/faucet.json
fi

if [[ -f $BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR" "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
else
  $nexis_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
fi

$nexis_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/vote-account.json
$nexis_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/stake-account.json

$nexis_ledger_tool create-snapshot \
...
$nexis_ledger_tool modify-genesis \
  --ledger "$SOLANA_CONFIG_DIR"/latest-mainnet-beta-snapshot \
  --hashes-per-tick sleep \

...
airdrops_enabled=1
node_sol=500 # 500 SOL: number of NZT to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-vlx NZT            - Number of NZT this node has been funded from the genesis config (default: $node_sol)
...
if [[ -n $SOLANA_CUDA ]]; then
  program=$nexis_validator_cuda
else
  program=$nexis_validator
fi
...
wallet() {
  (
    set -x
    $nexis_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
rpc_url=$($nexis_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $nexis_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $nexis_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $nexis_keygen new --no-passphrase -so "$authorized_withdrawer"

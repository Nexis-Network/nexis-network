...
ok=true
for program in nexis-{faucet,genesis,keygen,validator}; do
    $program -V || ok=false
done
$ok || {
    echo
...
if [[ -e $validator_vote_account ]]; then
    ...
    nexis-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
    ...
    nexis-keygen new --no-passphrase -so "$validator_stake_account"
fi
...
args=(
    ...
    --account-index nexis-accounts-storages
    --account-index nexis-accounts-owners
    --account-index nexis-accounts-operationals
    --evm-state-archive "$ledgerDir"/archive-evm
)
# shellcheck disable=SC2086
nexis-validator "${args[@]}" $SOLANA_RUN_SH_VALIDATOR_ARGS &
validator=$!

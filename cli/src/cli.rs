...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} NZT)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} NZT)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} NZT) + fee ({1} NZT)")]
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("nexis", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands

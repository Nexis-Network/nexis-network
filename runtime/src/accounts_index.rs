...
use nexis_account_program::{VAccountStorage, NexisAccountType};
...
pub enum IndexKey {
    ...
    NexisAccountStorage(Pubkey),
    NexisAccountOwner(Pubkey),
    NexisAccountOperational(Pubkey),
    NexisRelyingOwner(Pubkey),
}
...
impl AccountIndex {
    pub fn name(&self) -> &str {
        match self {
            ...
            AccountIndex::NexisAccountStorage => "Nexis Account Storage",
            AccountIndex::NexisAccountOwner => "Nexis Account Owner",
            AccountIndex::NexisAccountOperational => "Nexis Account Operational",
            AccountIndex::NexisRelyingOwner => "Nexis Relying Owner",
        }
    }
}
...
pub struct AccountsIndex<T: IndexValue> {
    ...
    // Nexis Indices
    nexis_account_storage_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    nexis_account_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    nexis_account_operational_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    nexis_relying_party_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
...
impl<T: IndexValue> AccountsIndex<T> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>) -> Self {
        ...
        Self {
            ...
            //Nexis indexes
            nexis_account_storage_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("nexis_account_storage_index"),
            nexis_account_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("nexis_account_owner_index"),
            nexis_account_operational_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("nexis_account_operational_index"),
            nexis_relying_party_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("nexis_relying_party_owner_index"),
        }
    }
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::NexisAccountStorage(va_storage_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.nexis_account_storage_index,
                    &va_storage_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::NexisAccountOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.nexis_account_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::NexisAccountOperational(va_operational_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.nexis_account_operational_index,
                    &va_operational_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::NexisRelyingOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.nexis_relying_party_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // Nexis account
        if *account_owner == nexis_account_program::id() {
            match NexisAccountType::try_from(account_data) {
                Ok(NexisAccountType::Account(account_info)) => {
                    if account_indexes.contains(&AccountIndex::NexisAccountStorage) {
                        let storage = account_info.find_storage_key(pubkey);
                        self.nexis_account_storage_index.insert(&storage, pubkey);
                    }

                    if account_indexes.contains(&AccountIndex::NexisAccountOwner) {
                        for owner in account_info.owners {
                            if owner != Pubkey::default() {
                                self.nexis_account_owner_index.insert(&owner, pubkey);
                            }
                        }
                    }
                }
                Ok(NexisAccountType::Storage(VAccountStorage { operationals })) => {
                    if account_indexes.contains(&AccountIndex::NexisAccountOperational) {
                ...
                Err(err) => log::warn!("Unable to parse Nexis Account: {:?}", err),
            }
        }

        // Nexis relying party data
        if *account_owner == nexis_relying_party_program::id() {
            match RelyingPartyData::try_from(account_data) {
                Ok(acc) => {
                    if account_indexes.contains(&AccountIndex::NexisRelyingOwner) {
                        self.nexis_relying_party_owner_index

...
use nexis_account_program::{NexisAccountType, ACCOUNT_LEN as NEXIS_ACCOUNT_SIZE};
use nexis_relying_party_program::RelyingPartyData;
...
impl JsonRpcRequestProcessor {
    ...
    fn get_nexis_accounts_by_storage_key(
        &self,
        bank: &Arc<Bank>,
        storage_key: Pubkey,
        ...
        let is_target_nexis_account = |account: &AccountSharedData| -> bool {
            *account.owner() == nexis_account_program::id()
                && account.data().len() == NEXIS_ACCOUNT_SIZE
                && matches!(
                    NexisAccountType::try_from(account.data()),
                    Ok(NexisAccountType::Account(_account_info))
                )
        };

        self.config
            .account_indexes
            .contains(&AccountIndex::NexisAccountStorage)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::NexisAccountStorage(storage_key),
                    is_target_nexis_account,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_nexis_accounts_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_nexis_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == nexis_account_program::id()
                && match NexisAccountType::try_from(account.data()) {
                    Ok(NexisAccountType::Account(account)) => account.owners.contains(&owner_key),
                    _ => false,
                }
        };
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::NexisAccountOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::NexisAccountOwner(owner_key),
                    is_target_nexis_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_nexis_relying_party_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_nexis_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == nexis_relying_party_program::id()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::NexisRelyingOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::NexisRelyingOwner(owner_key),
                    is_target_nexis_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
    ...
    fn get_nexis_accounts_storages_by_operational_key(
        &self,
        bank: &Arc<Bank>,
        ...
        let is_target_nexis_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == nexis_account_program::id()
                && match NexisAccountType::try_from(account.data()) {
                    Ok(NexisAccountType::Storage(account_storage)) => account_storage
                        .operationals
                        .iter()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::NexisAccountOperational)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::NexisAccountOperational(operational_key),
                    is_target_nexis_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `nexis-validator wait-for-restart-window` to not require this method, so
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        #[rpc(meta, name = "getNexisAccountsByOperationalKey")]
        fn get_nexis_accounts_by_operational_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;

        #[rpc(meta, name = "getNexisAccountsByOwnerKey")]
        fn get_nexis_accounts_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ...
        #[rpc(meta, name = "getNexisRelyingPartiesByOwnerKey")]
        fn get_nexis_relying_parties_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;
    ...
    impl AccountsData for AccountsDataImpl {
        ...
        fn get_nexis_accounts_by_owner_key(
            ...
            debug!(
                "get_nexis_accounts_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::NexisAccountOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta
                .get_nexis_accounts_by_owner_key(&bank, owner_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_nexis_accounts_by_owner_key nexis accounts {:?}",
                accounts
            );
        ...
        fn get_nexis_relying_parties_by_owner_key(
            ...
            debug!(
                "get_nexis_relying_parties_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::NexisRelyingOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta
                .get_nexis_relying_party_by_owner_key(&bank, owner_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_nexis_relying_parties_by_owner_key nexis accounts storages {:?}",
                accounts
            );
        ...
        fn get_nexis_accounts_by_operational_key(
            ...
            debug!(
                "get_nexis_accounts_by_operational_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::NexisAccountOperational);
            check_index!(&meta.config, AccountIndex::NexisAccountStorage);

            let operational_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let storages = meta
                .get_nexis_accounts_storages_by_operational_key(&bank, operational_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_nexis_accounts_by_operational_key nexis accounts storages {:?}",
                storages
            );

            let accounts: std::result::Result<Vec<_>, _> = storages
                .expect("nexis account operational index to be activated")
                .into_iter()
                .map(|(storage, _)| {
                    meta.get_nexis_accounts_by_storage_key(&bank, storage)
                        .expect("nexis account storage index to be activated")
                })
                .collect();
            ...
            debug!(
                "get_nexis_accounts_by_operational_key nexis accounts {:?}",
                accounts
            );

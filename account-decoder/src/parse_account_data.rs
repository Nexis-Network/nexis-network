...
lazy_static! {
    ...
    static ref NEXIS_ACCOUNT_PROGRAM_ID: Pubkey = nexis_account_program::id();
    static ref NEXIS_RELYING_PARTY_PROGRAM_ID: Pubkey = nexis_relying_party_program::id();
    pub static ref PARSABLE_PROGRAM_IDS: HashMap<Pubkey, ParsableAccount> = {
        ...
        m.insert(*NEXIS_ACCOUNT_PROGRAM_ID, ParsableAccount::NexisAccount);
        m.insert(
            *NEXIS_RELYING_PARTY_PROGRAM_ID,
            ParsableAccount::NexisRelyingParty,
        );
        m
    };
}
...
impl From<nexis_account_program::ParseError> for ParseAccountError {
    fn from(err: nexis_account_program::ParseError) -> Self {
        match err {
            nexis_account_program::ParseError::AccountNotParsable => {
                Self::AccountNotParsable(ParsableAccount::NexisAccount)
            }
        }
    }
}

impl From<nexis_relying_party_program::ParseError> for ParseAccountError {
    fn from(err: nexis_relying_party_program::ParseError) -> Self {
        match err {
            nexis_relying_party_program::ParseError::AccountNotParsable => {
                Self::AccountNotParsable(ParsableAccount::NexisRelyingParty)
            }
        }
    }
}

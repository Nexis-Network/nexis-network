...
impl<D: NexisDBCommon> Clone for Storage<D> {
    fn clone(&self) -> Self {
        Self {
...
impl<D: NexisDBCommon> std::fmt::Debug for Storage<D> {
...
static SECONDARY_MODE_PATH_SUFFIX: &str = "nexis-secondary";
...
pub trait NexisDBCommon: DBAccess + std::fmt::Debug + Sized {
...
impl NexisDBCommon for rocksdb::DBWithThreadMode<rocksdb::SingleThreaded> {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
impl NexisDBCommon for OptimisticTransactionDB {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
pub struct DbWithClose<D: NexisDBCommon>(D);
...
impl<D: NexisDBCommon> std::fmt::Debug for DbWithClose<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {

...
//!
//! 1. Generate a new keypair with `nexis-keygen new --outfile feature.json --no-passphrase`
...
lazy_static! {
    ...
    pub static ref FEATURE_NAMES: HashMap<Pubkey, &'static str> = FEATURE_NAMES_BEFORE_MAINNET.iter().map(|(k, v)| (*k, *v)).chain(
        [
            ...
            // nexis features
            (nexis::hardfork_pack::id(), "EVMblockhashes sysvar history, roothashes calculation. Apply old (reconfigure_native_token, unlock_switch_vote)."),
            (nexis::evm_cross_execution::id(), "EVM cross execution."),
            (nexis::native_swap_in_evm_history::id(), "Native swap in evm history."),
            (nexis::evm_new_error_handling::id(), "EVM new error handling."),
            (nexis::unsigned_tx_fix::id(), "Authorized transaction hash fixed."),
            (nexis::ignore_reset_on_cleared::id(), "Don't reset evm_swap address balance, when it already swapped, to avoid empty blocks."),
            (nexis::free_ownership_require_signer::id(), "Free ownership require signer."),
            (nexis::burn_fee::id(), "Burn fee during transaction execution."),
            (nexis::clear_logs_on_error::id(), "Clear logs from receipt if transaction is failed or reverted."),
            (nexis::disable_durable_nonce::id(), "Disable durable nonce."),
            (nexis::evm_new_precompiles::id(), "Evm new precomplies pack."),
            (nexis::evm_instruction_borsh_serialization::id(), "Support for Borsh serialization for EVM instructions."),
            (nexis::accept_zero_gas_price_with_native_fee::id(), "Accept evm transactions with native fee and zero gas price."),
            (nexis::clear_logs_on_native_error::id(), "Clear evm logs from receipt if native transaction is failed."),
            /*************** ADD NEW FEATURES HERE ***************/
        ]
    ).collect();

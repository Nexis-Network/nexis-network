...
USAGE:
    nexis-install-init [FLAGS] [OPTIONS] --data_dir <PATH> --pubkey <PUBKEY>
...
EOF
    ...
    main() {
        ...
        temp_dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t nexis-install-init)"
        ...
        download_url="$SOLANA_DOWNLOAD_ROOT/$release/nexis-install-init-$TARGET"
        solana_install_init="$temp_dir/nexis-install-init"
        ...
        if [ ! -x "$solana_install_init" ]; then
            ...
            printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./nexis-install-init." 1>&2
            exit 1
        fi
        ...
        if [ -z "$1" ]; then
            ...
            elif [ "$1" = "LATEST_NEXIS_RELEASE" ]; then
            ignore "$solana_install_init" $SOLANA_INSTALL_INIT_ARGS "$(echo $release | cut -c2-)"
        else
